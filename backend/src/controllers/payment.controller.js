...
//       const options = {
//     amount:((totalExpectedAmount-highestPricedBundle)*100),
//         currency: "INR",
...
// Full updated version with corrections and fixes
import { asynchHandler } from "../utils/AsynchHandler.js";
import { ApiError } from "../utils/ApiError.js";
import Payment from "../models/payment.model.js";
import { ApiResponse } from "../utils/ApiResponse.js";
...
const createPayment = asynchHandler(async (req, res) => {
  ...
  await newPayment.save();
  return res.status(200).json(new ApiResponse(true, "Payment created successfully", razorpayOrder));
});
...
const webHookHandler = asynchHandler(async (req, res) => {
  ...
  if (!isSignatureValid) return res.status(400).json(new ApiResponse(false, "Invalid webhook signature."));

  const event = req.body.event;
  const paymentDetails = req.body?.payload?.payment?.entity;
  const orderId = paymentDetails?.order_id;

  if (event !== 'payment.captured') return res.status(200).json(new ApiResponse(true, `Event '${event}' acknowledged.`));

  const payment = await Payment.findOne({ orderId });
  if (!payment) return res.status(200).json(new ApiResponse(true, "Acknowledged, but payment record not found."));

  if (payment.status === 'completed') return res.status(200).json(new ApiResponse(true, "Order already completed."));
  ...
  try {
    ...
    if (user.referredByCode && (payment.bundleIds.length > 0 || payment.courseIds.length > 0)) {
      const highestPricedItemAmount = payment.amount;
      const oneLevel = await User.findOne({ sharableReferralCode: user.referredByCode });
      if (oneLevel) {
        oneLevel.total_income += highestPricedItemAmount * 0.25;
        if (!oneLevel.myTeam.some(m => m.equals(user._id))) oneLevel.myTeam.push(user._id);
        ...
        oneLevel.incomeHistory.push({ 
          ...
          from: user._id,
          name: user.name,
          courseName: highestBundle?.bundleName || "N/A",
          isCourse: false,
        });
        await oneLevel.save({ validateBeforeSave: false });
        ...
        if (oneLevel.referredByCode) {
          ...
          if (twoLevel) {
            ...
            if (!twoLevel.myTeam.some(m => m.equals(user._id))) twoLevel.myTeam.push(user._id);
            twoLevel.totalTeam += 1;
            twoLevel.incomeHistory.push({ 
              amount: highestPricedItemAmount * 0.30, 
              from: user._id,
              name: user.name,
              courseName: highestBundle?.bundleName || "N/A",
              isCourse: false,
...
const deleteUnpaidUser = asynchHandler(async (req, res) => {
  const { email } = req.body;
  if (!email) throw new ApiError(400, "Email is required to delete an unpaid user.");
  const user = await User.findOne({ email });
  if (!user) return res.status(200).json(new ApiResponse(200, null, "User not found, no action needed."));
  await User.findByIdAndDelete(user._id);
