...
const webHookHandler = asynchHandler(async (req, res) => {
 ...
 try {
     ...
     const isSignatureValid = validateWebhookSignature(JSON.stringify(req.body), razorpaySignature, secret);
     ...
     if (event === 'order.paid') {
       ...
       if (!payment) {
         console.error(`Webhook Critical: Payment record not found for Order ID: ${orderEntity.id}. Manual check required.`);
       ...
       if (!user) {
           ...
           payment.notes.error = `User with email ${payment.notes.email} not found at time of fulfillment.`;
           await payment.save();
       ...
       try {
         ...
         console.error(`Webhook CRITICAL: Fulfillment failed for Order ID ${orderEntity.id}. Error: ${error.message}`);
       }
       ...
       console.log(`Webhook Info: Received unhandled event: ${event}`);
     }
